...
async function seedState() {
  const seedPath = path.join(process.cwd(), "data", "state.json");
  const publicSeedPath = path.join(process.cwd(), "public", "data", "state.json");
  for (const filePath of [seedPath, publicSeedPath]) {
    try {
      return JSON.parse(await fs.readFile(filePath, "utf8"));
    } catch {
      // Try the next seed path.
    }
  }
  return { days: {} };
}
