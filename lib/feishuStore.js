...
export async function getState({ force = false } = {}) {
  if (!force && cachedState && Date.now() < stateExpiresAt) return cachedState;
  ...
  cachedState = recordsToState(records, seed);
  stateExpiresAt = Date.now() + 30_000;
  return cachedState;
}
